...
class _ProductDetailPageState extends State<ProductDetailPage> {
  ...
  Future<void> _fetchUserRating() async {
    ...
    try {
      ...
      print(
        'Fetch user rating response: ${response.statusCode} - ${response.body}',
      );
  ...
  Future<void> _fetchNegotiatedPrice() async {
    ...
    try {
      ...
      print(
        'Fetch negotiated price response: ${response.statusCode} - ${response.body}',
      );
  ...
  Future<void> _submitRating(String productId, int rating) async {
    ...
    try {
      ...
      print(
        'Submit rating response: ${response.statusCode} - ${response.body}',
      );
      ...
      if (e.toString().contains('Access token missing') ||
  ...
  Future<void> _fetchUpdatedProductDetails(String productId) async {
    ...
    try {
      ...
      print(
        'Fetch updated product response: ${response.statusCode} - ${response.body}',
      );
      ...
      if (response.statusCode == 200) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              'Failed to fetch updated product details: ${response.body}',
            ),
          ),
        );
      }
  ...
  Future<void> _initiateChat() async {
    ...
    print(
      'Initiating chat - userId: $userId, token: $token, productId: ${widget.product['_id']}',
    );
    ...
    try {
      ...
      print(
        'Chat initiation response: ${response.statusCode} - ${response.body}',
      );
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('Error initiating chat: $e')));
      if (e.toString().contains('Access token missing') ||
          e.toString().contains('Invalid token')) {
        Navigator.pushReplacementNamed(context, '/login');
      }
  ...
  Widget build(BuildContext context) {
    ...
    final imageUrl =
        _currentProduct['image'] != null
            ...
            : null;
    final isInCart = clientState.cartProducts.any(
      (p) => p['_id'] == _currentProduct['_id'],
    );
    final availableQuantity = _currentProduct['quantity']?.toInt() ?? 0;
    final averageRating =
        _currentProduct['averageRating']?.toStringAsFixed(1) ?? '0.0';
    final ratingCount = _currentProduct['ratingCount']?.toString() ?? '0';
    final isSold =
        _currentProduct['productStatus'] == 'sold' || availableQuantity == 0;
                        ...
                        child: ElevatedButton(
                          onPressed: () {
                            ...
                            if (!isInCart) {
                              ScaffoldMessenger.of(context).showSnackBar(
                                SnackBar(
                                  content: Text(
                                    '${_currentProduct['title']} added to cart!',
                                  ),
                                  backgroundColor: Colors.green,
                                ),
                              );
                          ...
                          style: ElevatedButton.styleFrom(
                            backgroundColor:
                                isSold
                                    ? Colors.grey[400]
                                    : (isInCart ? Colors.grey : primaryColor),
                            foregroundColor: Colors.white,
