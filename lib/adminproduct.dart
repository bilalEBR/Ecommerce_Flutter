...
//                               : null;
//                       final firstLetter =
//                           product['title']?.toString().substring(0, 1) ?? 'P';
//                       return Container(
...
//         print(
//           'Update Product Response: ${response.statusCode} - ${responseBody.body}',
//         );
...
//             SnackBar(
//               content: Text(
//                 'Failed to update product: ${response.statusCode} - ${responseBody.body}',
//               ),
//             ),
//           );
//         }
...
//         final responseBody = await http.Response.fromStream(response);
//         print(
//           'Add Product Response: ${response.statusCode} - ${responseBody.body}',
//         );
...
//             SnackBar(
//               content: Text(
//                 'Failed to add product: ${response.statusCode} - ${responseBody.body}',
//               ),
//             ),
//           );
//         }
...
class _EditProductPageState extends State<EditProductPage> {
  ...
  Future<bool> _requestPermissions() async {
    ...
    if (status.isGranted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Photo library permission denied. Please allow access.')),
      );
      return false;
    } else if (status.isPermanentlyDenied) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Photo library permission permanently denied. Please enable in settings.')),
      );
  ...
  Future<void> _updateProduct() async {
    if (_formKey.currentState!.validate()) {
      ...
      try {
        ...
        // final token = Provider.of<ClientState>(context, listen: false).token;
        // if (token != null) {
        ...
        if (_selectedImageFile != null) {
          final bytes = _selectedImageFile!.bytes;
          if (bytes == null) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text('File data is unavailable')),
            );
          ...
          request.files.add(
            http.MultipartFile.fromBytes(
              ...
              contentType: MediaType(
                'image',
                _selectedImageFile!.extension?.toLowerCase() ?? 'jpeg',
              ),
            ),
          );
        }
