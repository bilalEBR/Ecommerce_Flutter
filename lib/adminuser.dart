...
Future<void> _deleteUser(String userId, Map<String, dynamic> user) async {
  try {
    final url = '$baseUrl/admin/users/users/$userId';
...
Future<void> _updateUser() async {
  if (_formKey.currentState!.validate()) {
    ...
    try {
      final url = '$baseUrl/admin/users/users/${widget.user['_id']}';
      ...
      if (response.statusCode == 200) {
        ...
        final errorData = jsonDecode(response.body);
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(errorData['error'] ?? 'Failed to update user')),
        );
      }
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
