  ...
  Future<void> fetchAccounts() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('Error fetching accounts: $e')));
      setState(() {
        isLoading = false;
      });
    }
    ...
    if (confirmDelete == true) {
      try {
        final response = await http.delete(
          Uri.parse('$baseUrl/adminAccount/$accountId'),
        );
        if (response.statusCode == 200) {
          ScaffoldMessenger.of(context).showSnackBar(
    ...
    setState(() {
      filteredAccounts =
          accounts
              .where(
                (account) =>
                    account['bankName'].toLowerCase().contains(
                      query.toLowerCase(),
                    ) ||
                    account['accountHolderName'].toLowerCase().contains(
                      query.toLowerCase(),
                    ),
              )
              .toList();
      ...
      try {
        if (widget.account != null) {
          ...
          if (response.statusCode == 201) {
            ...
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text('Failed to add account: ${response.body}'),
              ),
            );
          }
        }
            ...
            child: Column(
              ...
              children: [
                ...
                DropdownButtonFormField<String>(
                  ...
                  validator:
                      (value) => value == null ? 'Select account status' : null,
                  style: GoogleFonts.poppins(fontSize: 14),
                ),
